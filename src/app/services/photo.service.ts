...
export class PhotoService {
  ...
  public async addNewToGallery() {
    ...
    this.currentPhoto.unshift(savedImageFile);
    console.log('Fotos después de agregar:', this.currentPhoto);
  ...
  async deleteFirstPhoto() {
    try {
      if (this.photos.length > 0) {
        const photoToDelete = this.photos[0];
        ...
        // Si es una plataforma híbrida, ajusta la ruta de la foto
        if (this.platform.is('hybrid')) {
          photoToDelete.webviewPath = '';
        }

        // Elimina la primera foto del arreglo
        this.photos.shift();
        photoToDelete.webviewPath = '';
      }
    } catch (error) {
      console.error('Error al eliminar la primera foto', error);
    }
  }
  ...
  deleteAllPhotos() {
    ...
    // Limpia el arreglo de fotos
    this.photos = [];
  }

  getFirstPhotoFilePath(): string | null {
    return this.photos.length > 0 ? this.photos[0].filepath : null;
  }

  getFirstPhotoWebviewPath(): string | null {
    const firstPhoto = this.photos.length > 0 ? this.photos[0] : null;
    return firstPhoto ? firstPhoto.webviewPath || null : null;
  }
